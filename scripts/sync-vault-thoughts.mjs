...
const contentRoot = path.join(root, "content", "thoughts");
...
function fail(message) {
  console.error(`thought sync failed: ${message}`);
  process.exit(1);
}
...
function parseArgs(argv) {
  const options = {
    ...
    vaultRawBase:
      process.env.VAULT_THOUGHT_RAW_BASE ||
      process.env.VAULT_ARTICLE_RAW_BASE ||
      defaultVaultRawBase,
...
async function fetchText(url) {
  const response = await fetch(url, {
    headers: {
      "User-Agent": "HomePage thought sync",
...
async function run() {
  ...
  console.log(`thought sync complete: ${updates.length} updated, ${files.length} checked`);
}
